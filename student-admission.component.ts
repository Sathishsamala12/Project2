...
this.GetCountry()
  ...
  AddStudent():void{
   this. AddCurrent()
    ...
    this.studentAdmissionService.AddStudent(this.url+'AddStudent',this.studentDetail).subscribe((Add:any)=>{
      this.admissionlist=Add.StudentId;
...
generateAdmissiondetails(StudentId:number):any{
  if(StudentId>0){
  return 'sic'+new Date().getFullYear+StudentId;
}
  else
  {
  return null;
